...
export const metadata = { title: "Detail Hadits" };

export default async function StudentHaditsPage({
  params,
}: {
  params: Promise<{ studentId: string }>;
}) {
  const { studentId } = await params;
  const profile = await requireRole(["USTADZ"], `/ustadz/hadits/${studentId}`);
  ...
  const [entries, config] = await Promise.all([
    getLearningStudentAssessments(studentId, "HADITS"),
    getActiveLearningConfig(profile.tenantId, "HADITS"),
  ]);

  const lulus = entries.filter((e) => e.status === "LULUS").length;
  const latest = entries[0];
  const moduleLabel = LEARNING_MODULE_CONFIGS.HADITS.label;
